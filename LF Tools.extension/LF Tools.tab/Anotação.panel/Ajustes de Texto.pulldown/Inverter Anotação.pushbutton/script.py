...
# 3. Escolher direção - Dialog único e compacto
direction = forms.CommandSwitchWindow.show(
    ['↔ Horizontal', '↕ Vertical'],
    message='{}\nEscolha a direção:'.format(" + ".join(summary_items))
)

if not direction:
    script.exit()

# Normalizar nome da direção
...
with Transaction(doc, "Inverter Anotações") as t:
    ...
    for elem in annotations:
        if isinstance(elem, DB.FamilyInstance):
            try:
                if mirror_annotation(doc, elem, direction, active_view, error_log):
                    success_ann += 1
            except Exception as e:
    ...
    for elem in text_notes:
        try:
            if mirror_text_note(doc, elem, direction, active_view, error_log):
                success_txt += 1
        except Exception as e:
            error_log.append("TextNote ID {}: {}".format(elem.Id, str(e)))
