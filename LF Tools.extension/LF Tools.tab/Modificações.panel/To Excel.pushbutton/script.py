...
def get_panel_schedule_data(schedule_view):
    ...
    try:
        ...
        # Limpar referencia dos sistemas
        assigned_systems = None
...
def get_schedule_elements_and_params(schedule):
    """Extrai elementos e parametros de um schedule."""
    collector = None
    try:
    ...
    # Usar lista para evitar manter collector ativo
    collector = DB.FilteredElementCollector(doc, schedule.Id)
    elements = []
    
    for el in collector:
        try:
            if hasattr(el, 'Category') and el.Category and not el.Category.IsTagCategory: 
                elements.append(el)
        except: 
            continue
    
    # Limpar collector imediatamente
    collector = None
    
    if not elements: 
        return [], []
    
    # Processar parametros apenas dos primeiros 50 elementos para otimizar
...
def select_elements(elements):
    """Permite selecionar elementos da lista."""
    if not elements: 
        return []
    ...
    sel = forms.SelectFromList.show(
        sorted(defs, key=lambda x: x.label), 
    ...
    res = []
    for d in sel: 
        res.extend(d.elements)
    
    # Limpar mapa temporario
    type_element_map.clear()
    type_element_map = None
    defs = None
    
    return res

def select_parameters(src_elements, is_panel_schedule=False):
    """Permite selecionar parametros dos elementos."""
    param_defs_dict = {}
    non_storage_type = coreutils.get_enum_none(DB.StorageType)
    ...
    sel = forms.SelectFromList.show(
        sorted(param_defs_dict.values(), key=lambda x: x.name), 
        width=450, 
        multiselect=True, 
        title="Selecione Parametros"
    )
    
    # Limpar dicionario
    param_defs_dict.clear()
    param_defs_dict = None
...
def export_xls(src_elements, selected_params, file_path, is_panel_schedule=False):
    ...
    try:
        ...
        if is_panel_schedule:
            ...
            fmt_lock_ro = workbook.add_format({
                "locked": True, 
                "font_color": "#C0504D", 
                "italic": True
            })
            fmt_lock_id = workbook.add_format({
                "locked": True, 
                "font_color": "#95B3D7", 
                "italic": True
            })
            fmt_unlock = workbook.add_format({"locked": False})
            fmt_head_id = workbook.add_format({
                "bold": True, 
                "bg_color": "#DCE6F1", 
                "font_color": "#1F4E78"
            })
            ...
            # Processar em lotes para evitar sobrecarga de memoria
            batch_size = 500
            for batch_start in range(0, len(src_elements), batch_size):
                batch_end = min(batch_start + batch_size, len(src_elements))
                        ...
                        for c, p in enumerate(valid_params):
                            ...
                            if pval and pval.HasValue:
                                try:
                                    ...
                                    elif pval.StorageType == DB.StorageType.String: 
                                        val = pval.AsString()
                                    elif pval.StorageType == DB.StorageType.Integer:
                                        if is_yesno_parameter(p.definition): 
                                            val = "Yes" if pval.AsInteger() else "No"
                                        else: 
                                            val = str(pval.AsInteger())
                                    elif pval.StorageType == DB.StorageType.ElementId: 
                                        val = pval.AsValueString()
                                except: 
                                    val = "Error"
                ...
                # Forcar coleta de lixo a cada lote
                if batch_end < len(src_elements):
                    gc.collect()
            ...
            if len(src_elements) > 0: 
                ws.autofilter(0, 0, len(src_elements), len(valid_params))
        ...
        # GARANTIR que workbook seja fechado
        if workbook:
            try:
                workbook.close()
            except Exception as e:
                ...
                if "Permission denied" in msg or "being used" in msg or "Errno 32" in msg:
                    ...
                    logger.error("Erro ao fechar workbook: {}".format(e))
...
class ExportImportWindow(forms.WPFWindow):
    def __init__(self, xaml_file_path):
        ...
        # Registrar handlers para remover depois
        self._register_handler(self.Button_BrowseExport.Click, self.browse_export)
        self._register_handler(self.Button_BrowseImport.Click, self.browse_import)
        self._register_handler(self.Button_Export.Click, self.do_export)
        self._register_handler(self.Button_Import.Click, self.do_import)
        self._register_handler(self.Button_Close.Click, self.close_clicked)
        self._register_handler(self.ComboBox_ExportMode.SelectionChanged, self.mode_changed)
        
        self.load_schedules()
        self.mode_changed(None, None)
    
    def _register_handler(self, event, handler):
        """Registra event handler para remocao posterior."""
        event += handler
        self._event_handlers.append((event, handler))
    
    def _unregister_all_handlers(self):
        """Remove todos os event handlers."""
    ...
    def load_schedules(self):
        """Carrega schedules do documento."""
        self.view_schedules = []
        self.panel_schedules = []
        
        try:
            # View Schedules
            collector_views = DB.FilteredElementCollector(doc).OfClass(DB.ViewSchedule)
            for s in collector_views:
            ...
            # Limpar collector
            collector_views = None
            
            # Panel Schedules
            collector_panels = DB.FilteredElementCollector(doc).OfClass(DB.View)
            for v in collector_panels:
    ...
    def browse_export(self, sender, args):
        """Handler para botao de procurar arquivo de exportacao."""
    ...
    def browse_import(self, sender, args):
        """Handler para botao de procurar arquivo de importacao."""
    ...
    def do_export(self, sender, args):
        """Handler para botao de exportar."""
        ...
        try:
            ...
            elif idx == 2:  # Vista Ativa
                elements_in_view = revit.query.get_all_elements_in_view(active_view)
                src = select_elements(list(elements_in_view))
                elements_in_view = None
                if src: 
                    params = select_parameters(src)
                    
            elif idx == 3:  # Tipos de Familia
                all_els = revit.query.get_all_elements(doc)
                type_els = [e for e in all_els if DB.ElementIsElementTypeFilter().PassesFilter(e)]
                all_els = None
                src = select_elements(type_els)
                type_els = None
                if src: 
                    params = select_parameters(src)
                    
            elif idx == 4:  # Instancias
                all_els = revit.query.get_all_elements(doc)
                instance_els = [e for e in all_els if DB.ElementIsElementTypeFilter(True).PassesFilter(e)]
                all_els = None
                src = select_elements(instance_els)
                instance_els = None
                if src: 
                    params = select_parameters(src)
                    
            elif idx == 5:  # Selecao Manual
                self.Hide()
                picked_els = revit.pick_elements()
                self.Show()
                src = select_elements(list(picked_els))
                picked_els = None
                if src: 
                    params = select_parameters(src)
            ...
            # LIMPEZA CRITICA - liberar todas as referencias
            src = None
            params = None
            gc.collect()

    def do_import(self, sender, args):
        """Handler para botao de importar."""
    ...
    def cleanup_and_close(self):
        ...
        try:
            ...
            # Limpar referencias de controles
            self.Button_BrowseExport = None
            self.Button_BrowseImport = None
            self.Button_Export = None
            self.Button_Import = None
            self.Button_Close = None
            self.ComboBox_ExportMode = None
            self.ComboBox_SubSelection = None
            self.TextBox_ExportPath = None
            self.TextBox_ImportPath = None
            self.TextBlock_Status = None
            
            # Forcar coleta de lixo
            gc.collect()
            
        except Exception as e:
            logger.error("Erro na limpeza: {}".format(e))
        finally:
            try:
                self.Close()
            except:
...
if __name__ == "__main__":
    ...
    if os.path.exists(xaml_file):
        ...
        try:
            ...
            logger.error("Erro na janela: {}".format(e))
            logger.error(traceback.format_exc())
        finally:
        ...
        forms.alert("Arquivo XAML nao encontrado: {}".format(xaml_file), title="Erro")