...
class _WPFWindowCPy:
    ...
    def __init__(self, xaml_source, literal_string=None):
        ...
        # No CPython, é crucial definir o Owner para evitar que a janela 
        # se perca no background ou cause erros de thread.
        try:
            from pyrevit.coreutils import ribbon
            main_window = ribbon.get_main_window()
            if main_window:
                self._window.Owner = main_window
        except:
            pass
...
# --- DETECAO DE VERSAO ---
# Usamos HOST_APP.version para evitar erro se o documento demorar a carregar no CPython
try:
    from pyrevit import revit, HOST_APP
    REVIT_YEAR = int(HOST_APP.version)
except:
    REVIT_YEAR = 2024 # Fallback

# --- INICIALIZAÇÃO RESILIENTE DO DOCUMENTO (CPython/Revit 2025 Deep Search) ---
doc = None
uidoc = None
...
try:
    ...
    # 2. SE FALHAR: Busca Profunda na lista de documentos abertos da aplicação
    if not doc:
        uiapp = __revit__
        # Tenta pegar qualquer projeto aberto (ignora famílias)
        try:
            for d_open in uiapp.Application.Documents:
                if not d_open.IsFamilyDocument:
                    doc = d_open
                    break
            # Tenta capturar o UIDocument correspondente
            if doc:
...
class LuisExporterWindow(forms.WPFWindow):
    ...
    def _hard_refresh(self):
        ...
        try:
            ...
            self.dg_Sheets.ItemsSource = None
    ...
    def on_row_clicked(self, sender, args):
        """Desativado: O clique agora é gerenciado nativamente pelo CheckBox no XAML"""
        pass
    ...
    def on_pattern_changed(self, sender, args):
        ...
        # No CPython, para Texto, o Refresh é melhor que o Reset total
        self.dg_Sheets.Items.Refresh()
